...
# maturity: 2
# domain: governance
# test_path: tests/bats/test_check_session_writer.bats
# risk_profile:
...
# test:
#   runner: bats
#   command: bats tests/bats/test_check_session_writer.bats
...
# Check for staged agent report files (new or modified) — both canonical and legacy paths
STAGED_REPORTS=$(git diff --cached --name-only --diff-filter=AM -- 'agent-reports/*.md' '.claude/agent-reports/*.md' | grep -v 'README.md' || true)
